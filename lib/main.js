    ...
    function (data) {
      ...
      for (index = 0; index < data.Reservations.length && !ipAddress; ++index) {
        for (jndex = 0; jndex < data.Reservations[index].Instances.length && !ipAddress; ++jndex) {
          if (process.env.LAMBDA_TASK_ROOT) {    
            ipAddress = data.Reservations[index].Instances[jndex].PrivateIpAddress;
          } else {
      ...
      console.log("Waiting for Network Availablity on ", ipAddress, "...");
      
      start = new Date();
      return require('./waitforport')(ipAddress, { numRetries: 8, retryInterval: 30000 });
    }
  ).then(
    function () {
